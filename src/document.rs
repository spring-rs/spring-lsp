        ...
        if let Some(mut doc) = self.documents.get_mut(uri) {
            ...
            for change in changes {
                if let Some(range) = change.range {
                    // 增量修改
                    if let Err(e) = Self::apply_incremental_change(&mut doc.content, range, &change.text) {
    ...
    fn apply_incremental_change(
        ...
        if start_offset > end_offset || end_offset > content.len() {
            return Err(format!(
                ...
                start_offset, end_offset, content.len()
            ));
        }
        
        // 构建新内容
        let mut new_content = String::with_capacity(
            content.len() - (end_offset - start_offset) + text.len()
        );
...
mod tests {
    ...
    fn test_incremental_change_multiline() {
        ...
        manager.open(uri.clone(), 1, initial_content.to_string(), "toml".to_string());
    ...
    fn test_with_document() {
        ...
        let not_found = manager.with_document(&"file:///notfound.toml".parse().unwrap(), |doc| doc.content.len());
    ...
    fn test_position_to_offset() {
        ...
        // 第一行开始
        let offset = DocumentManager::position_to_offset(content, Position { line: 0, character: 0 }).unwrap();
        assert_eq!(offset, 0);
        
        // 第一行 "line" 后
        let offset = DocumentManager::position_to_offset(content, Position { line: 0, character: 4 }).unwrap();
        assert_eq!(offset, 4);
        
        // 第二行开始
        let offset = DocumentManager::position_to_offset(content, Position { line: 1, character: 0 }).unwrap();
        assert_eq!(offset, 7); // "line 1\n"
        
        // 第三行开始
        let offset = DocumentManager::position_to_offset(content, Position { line: 2, character: 0 }).unwrap();
