...
use lsp_types::{
    ...
    TextDocumentContentChangeEvent, Position, WorkDoneProgressParams,
};
...
"#.to_string(),
    ...
    let completions = server.document_manager.with_document(&uri, |doc| {
        if let Ok(toml_doc) = server.toml_analyzer.parse(&doc.content) {
            server.completion_engine.complete_toml_document(&toml_doc, Position { line: 3, character: 0 })
        } else {
            vec![]
        }
    });
    ...
    if let Some(completions) = completions {
        ...
        println!("No completion suggestions (this may be normal if schema doesn't define the section)");
    }
}
...
fn test_completion_performance_with_document_sizes() {
    ...
    for (size_name, section_count) in sizes {
        println!("Testing {} document ({} sections)...", size_name, section_count);
...
"#,
        ...
        let completions = server.document_manager.with_document(&uri, |doc| {
            if let Ok(toml_doc) = server.toml_analyzer.parse(&doc.content) {
                ...
                server.completion_engine.complete_toml_document(&toml_doc, Position { 
...
"#;
    ...
    let test_positions = vec![
        (Position { line: 4, character: 0 }, "web section completion"),
        (Position { line: 20, character: 0 }, "redis section completion"),
        (Position { line: 35, character: 0 }, "postgres section completion"),
        (Position { line: 70, character: 0 }, "new section completion"),
    ];
...
"#.to_string(),
    ...
    // 计算平均缓存时间
    let avg_cached_duration = cached_durations.iter().sum::<Duration>() / cached_durations.len() as u32;
...
"#;
    ...
    assert!(!diagnostics.is_empty(), "Should generate diagnostics for errors");
...
"#);
    ...
    // 验证诊断结果
    let diagnostics = server.diagnostic_engine.get(&uri);
    println!("Generated diagnostics for large document: {}", diagnostics.len());
...
fn test_multi_document_diagnostic_performance() {
    ...
    // 创建多个文档
    let documents = vec![
        ("file:///multi-doc-1.toml", r#"
[web]
host = "localhost"
port = 8080
"#),
        ("file:///multi-doc-2.toml", r#"
[redis]
url = "redis://localhost:6379"
pool_size = 10
"#),
        ("file:///multi-doc-3.toml", r#"
[postgres]
url = "postgresql://localhost/db"
max_connections = 20
"#),
        ("file:///multi-doc-4.toml", r#"
[mail]
smtp_host = "smtp.gmail.com"
smtp_port = 587
"#),
        ("file:///multi-doc-5.toml", r#"
[job]
enabled = true
thread_pool_size = 4
"#),
...
"#;
    ...
    for i in 0..5 {
        ...
        server.handle_did_change(DidChangeTextDocumentParams {
            text_document: VersionedTextDocumentIdentifier {
                uri: main_config_uri.clone(),
                version: i + 2,
            },
        ...
        // 请求补全
        let _completions = server.document_manager.with_document(&main_config_uri, |doc| {
            if let Ok(toml_doc) = server.toml_analyzer.parse(&doc.content) {
                server.completion_engine.complete_toml_document(&toml_doc, Position { 
...
fn test_performance_regression() {
    ...
    for i in 0..5 {
        ...
        let _completions = server.document_manager.with_document(&uri, |doc| {
            if let Ok(toml_doc) = server.toml_analyzer.parse(&doc.content) {
                server.completion_engine.complete_toml_document(&toml_doc, Position { line: 2, character: 0 })
            } else {
                vec![]
            }
        });
