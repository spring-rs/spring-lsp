...
fn property_schema() -> impl Strategy<Value = PropertySchema> {
    (
        ...
        .prop_map(
            |(name, type_info, description, default, required, deprecated)| PropertySchema {
                name: name.clone(),
                type_info,
                description,
                default,
...
fn config_schema() -> impl Strategy<Value = ConfigSchema> {
    prop::collection::hash_map(valid_prefix(), plugin_schema(), 0..10).prop_map(|plugins| {
