...
"#;
    ...
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("未在 Schema 中定义")
    });
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("类型不匹配")
    });
...
"#;
    ...
    let warning = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::WARNING) && d.message.contains("缺少必需的配置项")
    });
...
"#;
    ...
    let warning = diagnostics
        .iter()
        .find(|d| d.severity == Some(DiagnosticSeverity::WARNING) && d.message.contains("已废弃"));
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("环境变量名不能为空")
    });
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let warning = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::WARNING) && d.message.contains("不符合命名规范")
    });
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("小于最小值")
    });
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("超过最大值")
    });
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("小于最小长度")
    });
...
"#;
    ...
    assert!(!diagnostics.is_empty());
    let error = diagnostics.iter().find(|d| {
        d.severity == Some(DiagnosticSeverity::ERROR) && d.message.contains("不在允许的枚举值中")
    });
...
"#;
    ...
    // 不应该有环境变量相关的错误
    let env_errors = diagnostics
        .iter()
        .filter(|d| d.message.contains("环境变量"))
        .count();
