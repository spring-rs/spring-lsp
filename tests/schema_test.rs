...
fn test_config_schema_serialization() {
    ...
    assert_eq!(host_prop.default, Some(Value::String("localhost".to_string())));
...
fn test_type_info_variants() {
    ...
    if let TypeInfo::String { enum_values, min_length, max_length } = deserialized {
        assert_eq!(enum_values, Some(vec!["dev".to_string(), "prod".to_string()]));
...
fn test_deprecated_property() {
    ...
    assert_eq!(deserialized.deprecated, Some("请使用 new_config 代替".to_string()));
}
