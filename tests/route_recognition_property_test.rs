...
fn route_path_with_params() -> impl Strategy<Value = (String, Vec<String>)> {
    prop::sample::select(vec![
        ...
        ("/users/{user_id}/posts/{post_id}".to_string(), vec!["user_id".to_string(), "post_id".to_string()]),
        ("/api/v1/users/{id}".to_string(), vec!["id".to_string()]),
        ("/posts/{post_id}/comments/{comment_id}".to_string(), vec!["post_id".to_string(), "comment_id".to_string()]),
        ("/api/{version}/users/{id}".to_string(), vec!["version".to_string(), "id".to_string()]),
    ])
}
...
fn multiple_http_methods() -> impl Strategy<Value = Vec<HttpMethod>> {
    prop::collection::vec(single_http_method(), 1..4)
        .prop_map(|methods| {
            // 去重
...
fn valid_range() -> impl Strategy<Value = Range> {
    (0u32..1000, 0u32..100, 0u32..1000, 0u32..100)
        .prop_map(|(start_line, start_char, end_line, end_char)| {
...
fn single_route_macro() -> impl Strategy<Value = RouteMacro> {
    (
        ...
        .prop_map(|(path, methods, handler_name, range)| {
            RouteMacro {
                path,
                methods,
                middlewares: vec![],
...
fn rust_document_with_routes() -> impl Strategy<Value = RustDocument> {
    (
        ...
        .prop_map(|(uri, route_macros)| {
            let macros = route_macros
                .into_iter()
                .map(SpringMacro::Route)
                .collect();
